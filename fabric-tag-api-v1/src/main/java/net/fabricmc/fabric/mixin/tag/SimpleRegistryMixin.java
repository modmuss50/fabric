...
abstract class SimpleRegistryMixin<T> implements SimpleRegistryExtension, TagAliasEnabledRegistryWrapper {
	...
	public void fabric_applyPendingTagAliases() {
		...
		for (Set<TagKey<?>> aliasGroup : uniqueAliasGroups) {
			...
			for (TagKey<?> tag : aliasGroup) {
				...
				if (entryList != null) {
					...
					LOGGER.info("[Fabric] Creating a new empty tag {} for unknown tag used in a tag alias group in {}", tag.location(), tag.registry().location());
		...
		LOGGER.debug("[Fabric] Loaded {} tag alias groups for {}", uniqueAliasGroups.size(), key.location());
